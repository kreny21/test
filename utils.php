...
class DBUtils{
    ...
    public static function is_user_reviewed(PDO $db, $client_id):bool {
        $check_query = $db->prepare('select client_id from reviews where client_id = ? limit 1');
        $check_query->execute([$client_id]);
