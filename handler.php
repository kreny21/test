...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    $client_id = filter_var($_GET['client_id'],FILTER_VALIDATE_INT);
    if($client_id){
        require_once 'config.php';
        require_once 'utils.php';
        try{
            if(DBUtils::is_user_exist($db,$client_id)){
                if (DBUtils::is_user_reviewed($db,$client_id)) {
                ...
                if(in_array($grade,['1','2','3','4','5'])){
                    $insert_query = $db->prepare('insert into reviews(client_id, grade, comment) values (?, ?, ?)');
                    $insert_query->execute([$client_id, $grade, $comment]);
